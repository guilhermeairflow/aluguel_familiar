...
const INITIAL_BOOKINGS: Booking[] = [
  ...
  { id: 3,  propId: '2', guest: 'Patrícia Lima',   phone: '11966665555', email: '', checkin: '2026-02-01', checkout: '2026-02-08', guests: 35, value: 32100, cleaningFee: 600, status: 'Confirmado', notes: '', createdAt: '2025-12-01' },
  ...
  { id: 11, propId: '1', guest: 'Família Torres',  phone: '11977661111', email: '', checkin: '2026-07-04', checkout: '2026-07-12', guests: 28, value: 40000, cleaningFee: 500, status: 'Confirmado', notes: '', createdAt: '2026-03-05' },
  ...
  { id: 14, propId: '7', guest: 'Réveillon Group', phone: '11944334444', email: '', checkin: '2026-12-26', checkout: '2027-01-02', guests: 18, value: 46200, cleaningFee: 700, status: 'Pendente',   notes: '', createdAt: '2026-03-10' },
  { id: 15, propId: '1', guest: 'Família Alves',   phone: '11933225555', email: '', checkin: '2026-12-26', checkout: '2027-01-02', guests: 30, value: 57600, cleaningFee: 500, status: 'Pendente',   notes: '', createdAt: '2026-03-12' },
];
...
function WorkflowModal({ onClose, onSave }: { onClose: () => void; onSave: (b: NewBooking) => void }) {
  ...
  const steps = ['Imóvel', 'Hóspede', 'Datas', 'Financeiro', 'Confirmação'];

  // Auto-preenche preço base ao selecionar imóvel
  useEffect(() => {
    if (form.propId) {
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 20, width: '100%', maxWidth: 540, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ padding: '22px 28px 0', borderBottom: '1px solid #f1f5f9' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            <div>
              <div style={{ fontWeight: 900, fontSize: '1.1rem', color: '#0f172a' }}>Nova Locação</div>
              <div style={{ fontSize: '0.78rem', color: '#94a3b8', marginTop: 2 }}>Passo {step} de {steps.length}</div>
            </div>
            <button onClick={onClose} style={{ background: '#f1f5f9', border: 'none', borderRadius: 8, width: 34, height: 34, cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
          </div>
        ...
        <div style={{ padding: '24px 28px' }}>
          ...
          {step === 1 && (
            <div>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: '#0f172a', marginBottom: 4 }}>🏠 Selecione o Imóvel</div>
              <div style={{ color: '#64748b', fontSize: '0.82rem', marginBottom: 16 }}>Qual propriedade será locada?</div>
                  ...
                  return (
                    <button key={p.id} onClick={() => set('propId', p.id)} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '12px 14px', borderRadius: 12, border: `2px solid ${isSelected ? color : '#e2e8f0'}`, background: isSelected ? `${color}10` : 'white', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: 2 }}>📍 {p.city} · 👥 até {p.maxGuests} hósp. · 💰 R$ {p.basePricePerNight.toLocaleString('pt-BR')}/noite</div>
                      </div>
                      {isSelected && <span style={{ color, fontSize: '1.2rem' }}>✓</span>}
                    </button>
                  );
                })}
              </div>
          ...
          {step === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: '#0f172a', marginBottom: 4 }}>👤 Dados do Hóspede</div>
              <div>
                <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: 6 }}>Nome Completo *</label>
                <input style={inputCls} placeholder="Ex: João da Silva" value={form.guest} onChange={e => set('guest', e.target.value)} />
              </div>
              <div>
                ...
                <div style={{ display: 'flex', border: '1.5px solid #e2e8f0', borderRadius: 8, overflow: 'hidden' }}>
                  <span style={{ padding: '10px 12px', background: '#f8fafc', color: '#64748b', fontWeight: 700, borderRight: '1px solid #e2e8f0', fontSize: '0.85rem' }}>🇧🇷 +55</span>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <button onClick={() => set('guests', Math.max(1, form.guests - 1))} style={{ width: 38, height: 38, borderRadius: 8, border: '1.5px solid #e2e8f0', background: 'white', fontSize: '1.2rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#475569' }}>−</button>
                  ...
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>máx. {selectedProp?.maxGuests}</span>
                </div>
              </div>

              {/* Crianças */}
              <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 14 }}>
                <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: 6 }}>Crianças (0-17 anos)</label>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: form.children > 0 ? 14 : 0 }}>
                  ...
                  <button
                    ...
                    style={{ width: 38, height: 38, borderRadius: 8, border: '1.5px solid #e2e8f0', background: 'white', fontSize: '1.2rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#475569' }}>+</button>
                  <span style={{ fontSize: '0.8rem', color: '#64748b' }}>criança{form.children !== 1 ? 's' : ''}</span>
                </div>
                ...
                {form.children > 0 && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(110px, 1fr))', gap: 10 }}>
                    {form.childrenAges.map((age, i) => (
                      <div key={i}>
                        <label style={{ display: 'block', fontSize: '0.72rem', fontWeight: 700, color: '#64748b', marginBottom: 4 }}>Criança {i + 1} — Idade</label>
                        <div style={{ display: 'flex', border: '1.5px solid #e2e8f0', borderRadius: 8, overflow: 'hidden' }}>
                          <input
                ...
                {form.children > 0 && (
                  <div style={{ marginTop: 10, padding: '8px 12px', background: '#faf5ff', borderRadius: 8, border: '1px solid #e9d5ff', fontSize: '0.76rem', color: '#7c3aed', fontWeight: 600 }}>
                    👶 {form.children} criança{form.children !== 1 ? 's' : ''} · Idades: {form.childrenAges.map(a => `${a} ano${a !== 1 ? 's' : ''}`).join(', ')}
                  </div>
                )}
              </div>
            </div>
          ...
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: '#0f172a', marginBottom: 4 }}>📅 Período da Locação</div>
                ...
                <div style={{ background: '#eff6ff', borderRadius: 12, padding: '14px 18px', border: '1px solid #bfdbfe' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div style={{ background: 'white', borderRadius: 10, padding: '12px 14px', border: '1px solid #bfdbfe', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '0.04em' }}>🌙 Pernoites</div>
                    ...
                    <div style={{ background: 'white', borderRadius: 10, padding: '12px 14px', border: '1px solid #bfdbfe', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#7c3aed', textTransform: 'uppercase', letterSpacing: '0.04em' }}>☀️ Dias de Uso</div>
          ...
          {step === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: '#0f172a', marginBottom: 4 }}>💰 Valores e Pagamento</div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div>
                  <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: 6 }}>Diária (R$) *</label>
                  <div style={{ display: 'flex', border: '1.5px solid #e2e8f0', borderRadius: 8, overflow: 'hidden' }}>
              ...
              <div style={{ background: '#f8fafc', borderRadius: 12, padding: '16px 18px', border: '1px solid #e2e8f0' }}>
                <div style={{ fontWeight: 700, fontSize: '0.85rem', color: '#0f172a', marginBottom: 12, borderBottom: '1px solid #e2e8f0', paddingBottom: 10 }}>📋 Resumo Financeiro</div>
                {[
                  { label: `${nights} noite${nights !== 1 ? 's' : ''} × R$ ${form.dailyRate.toLocaleString('pt-BR')}`, value: form.dailyRate * nights },
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 700, color: '#374151', marginBottom: 6 }}>Observações / Anotações</label>
                <textarea
          ...
          {step === 5 && (
            <div>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: '#0f172a', marginBottom: 16 }}>✅ Revisão Final</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ background: '#f8fafc', borderRadius: 12, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                  <div style={{ display: 'flex', gap: 0 }}>
                    ...
                    <div style={{ padding: '10px 14px' }}>
                      ...
                      <div style={{ fontSize: '0.75rem', color: '#64748b', marginTop: 2 }}>{selectedProp?.maxGuests} hósp. máx.</div>
                    </div>
                  </div>
                </div>

                {[
                  { label: '👤 Responsável', value: form.guest },
                  { label: '📱 WhatsApp', value: `+55 ${form.phone}` },
                  { label: '🧑 Adultos', value: `${form.guests} pessoa${form.guests !== 1 ? 's' : ''}` },
                  { label: '👶 Crianças', value: form.children === 0 ? 'Nenhuma' : `${form.children} criança${form.children !== 1 ? 's' : ''} · Idades: ${form.childrenAges.map(a => `${a}a`).join(', ')}` },
                  { label: '📅 Check-in', value: fmtDate(form.checkin) },
                  { label: '📅 Check-out', value: fmtDate(form.checkout) },
                  { label: '🌙 Pernoites', value: `${nights} noite${nights !== 1 ? 's' : ''}` },
                  { label: '☀️ Dias de Uso', value: `${nights + 1} dia${(nights + 1) !== 1 ? 's' : ''} (inclui dia de chegada)` },
                ...
                <div style={{ background: '#f0fdf4', borderRadius: 12, padding: '14px', border: '1px solid #bbf7d0', marginTop: 4 }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#15803d', marginBottom: 8 }}>📱 Mensagem WhatsApp (gerada automaticamente ao salvar)</div>
                  <div style={{ fontSize: '0.78rem', color: '#166534', lineHeight: 1.6, background: 'white', borderRadius: 8, padding: '10px 12px', border: '1px solid #d1fae5' }}>
                    Olá {form.guest.split(' ')[0]}! 😊 Reserva confirmada em *{selectedProp?.city}* de *{fmtDate(form.checkin)}* a *{fmtDate(form.checkout)}* ({nights} noites, {form.guests} adulto{form.guests !== 1 ? 's' : ''}{form.children > 0 ? ` e ${form.children} criança${form.children !== 1 ? 's' : ''}` : ''}). Valor total: *R$ {total.toLocaleString('pt-BR')}*. Qualquer dúvida estamos à disposição! — AluguelFamiliar.
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 20 }}>
      ...
      <div style={{ background: 'white', borderRadius: 14, border: '1px solid #e2e8f0', padding: '16px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ fontSize: '0.7rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '0 6px', marginBottom: 2 }}>Filtrar Imóvel</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ background: 'white', borderRadius: 14, border: '1px solid #e2e8f0', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: '0.78rem', color: '#64748b', marginTop: 2 }}>{monthBookings.length} reserva{monthBookings.length !== 1 ? 's' : ''} neste mês</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={prevMonth} style={{ width: 34, height: 34, borderRadius: 8, border: '1.5px solid #e2e8f0', background: 'white', cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>‹</button>
            <button onClick={nextMonth} style={{ width: 34, height: 34, borderRadius: 8, border: '1.5px solid #e2e8f0', background: 'white', cursor: 'pointer', fontSize: '1.1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
          </div>
        </div>
                    ...
                    {dayBkgs.slice(0, 2).map(b => {
                      ...
                      return (
                        <div key={b.id} style={{ background: `${color}22`, borderLeft: `2.5px solid ${color}`, borderRadius: '0 3px 3px 0', padding: '2px 4px', fontSize: '0.6rem', fontWeight: 700, color, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 3 }}>
                          {isStart && <span style={{ fontSize: '0.55rem' }}>↘</span>}
                          {isEnd && <span style={{ fontSize: '0.55rem' }}>↗</span>}
                          <span>{firstName}</span>
                          <span style={{ opacity: 0.7, fontSize: '0.55rem' }}>· {b.guests}👥</span>
                        </div>
                      );
                    })}
        ...
        {monthBookings.length > 0 && (
          <div style={{ background: 'white', borderRadius: 14, border: '1px solid #e2e8f0', padding: '16px 20px' }}>
            <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#0f172a', marginBottom: 12 }}>📋 Reservas em {MONTHS_FULL[month]}</div>
                ...
                return (
                  <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#f8fafc', borderRadius: 10, borderLeft: `4px solid ${color}`, flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 100 }}>
                      ...
                      <div style={{ fontSize: '0.72rem', color: '#64748b' }}>{b.guest} · {b.guests} hósp. · {nights}n</div>
                    </div>
                    ...
                    <a href={`https://wa.me/55${b.phone}?text=Olá ${b.guest.split(' ')[0]}! Aqui é da equipe AluguelFamiliar.`} target="_blank" rel="noreferrer" style={{ background: '#25D366', color: 'white', padding: '5px 10px', borderRadius: 7, textDecoration: 'none', fontWeight: 700, fontSize: '0.72rem' }}>
                      WhatsApp
                    </a>
                  </div>
                );
        ...
        <div onClick={() => setShowDayModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 26, width: '100%', maxWidth: 460, maxHeight: '80vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              ...
              <button onClick={() => setShowDayModal(false)} style={{ background: '#f1f5f9', border: 'none', borderRadius: 8, width: 32, height: 32, cursor: 'pointer', fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
            </div>
            {selectedBookings.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '28px 0', color: '#94a3b8' }}>
                <div style={{ fontSize: '2.5rem' }}>📭</div>
                <div style={{ fontWeight: 600, marginTop: 8 }}>Dia disponível</div>
                <button onClick={() => { setShowDayModal(false); setShowNewBooking(true); }} style={{ marginTop: 14, background: '#0f172a', color: 'white', border: 'none', padding: '9px 20px', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: '0.88rem' }}>+ Cadastrar Locação</button>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  return (
                    <div key={b.id} style={{ borderRadius: 14, border: `2px solid ${color}33`, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                          ...
                          <div style={{ padding: '10px 14px', flex: 1 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'flex-end' }}>
                                {isCheckin && <span style={{ background: '#10b981', color: 'white', padding: '2px 8px', borderRadius: 20, fontSize: '0.65rem', fontWeight: 700, whiteSpace: 'nowrap' }}>✈️ CHECK-IN</span>}
                                {isCheckout && <span style={{ background: '#f59e0b', color: 'white', padding: '2px 8px', borderRadius: 20, fontSize: '0.65rem', fontWeight: 700, whiteSpace: 'nowrap' }}>🏠 CHECK-OUT</span>}
                      ...
                      <div style={{ padding: '14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        {[
                          { icon: '👤', label: 'Hóspede Resp.', value: b.guest },
                          { icon: '📱', label: 'WhatsApp', value: `+55 ${b.phone}` },
                          { icon: '📅', label: 'Check-in', value: fmtDate(b.checkin) },
                          { icon: '📅', label: 'Check-out', value: fmtDate(b.checkout) },
                          { icon: '🌙', label: 'Pernoites', value: `${nights} noite${nights !== 1 ? 's' : ''}` },
                          { icon: '☀️', label: 'Dias de Uso', value: `${nights + 1} dia${(nights + 1) !== 1 ? 's' : ''}` },
                          { icon: '🧑', label: 'Adultos', value: `${b.guests} pessoa${b.guests !== 1 ? 's' : ''}` },
                          { icon: '👶', label: 'Crianças', value: (b as any).children > 0 ? `${(b as any).children} · ${((b as any).childrenAges || []).map((a: number) => `${a}a`).join(', ')}` : 'Nenhuma' },
                          { icon: '👥', label: 'Total Pessoas', value: `${totalPeople} no total` },
                          { icon: '💰', label: 'Valor Total', value: `R$ ${b.value.toLocaleString('pt-BR')}` },
                        ].map((row, i) => (
                      ...
                      <div style={{ padding: '0 14px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <div style={{ flex: 1, textAlign: 'center', background: b.status === 'Confirmado' ? '#f0fdf4' : '#fffbeb', color: b.status === 'Confirmado' ? '#15803d' : '#d97706', padding: '8px', borderRadius: 8, fontWeight: 700, fontSize: '0.82rem', border: `1px solid ${b.status === 'Confirmado' ? '#bbf7d0' : '#fde68a'}` }}>
                            {b.status === 'Confirmado' ? '✅' : '⏳'} {b.status}
                          </div>
                          <a href={`https://wa.me/55${b.phone}?text=Olá ${b.guest.split(' ')[0]}! Aqui é a equipe AluguelFamiliar. Sua reserva em *${prop?.city}* de *${fmtDate(b.checkin)}* a *${fmtDate(b.checkout)}* está confirmada! Qualquer dúvida estamos à disposição. 😊`} target="_blank" rel="noreferrer" style={{ flex: 1, textAlign: 'center', background: '#25D366', color: 'white', padding: '8px', borderRadius: 8, textDecoration: 'none', fontWeight: 700, fontSize: '0.82rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                            📱 WhatsApp
                          </a>
                        </div>
                        {(b as any).notes && (
      ...
      {/* Modal: Nova Locação */}
      {showNewBooking && <WorkflowModal onClose={() => setShowNewBooking(false)} onSave={handleSaveBooking} />}
    </div>
  );
}
